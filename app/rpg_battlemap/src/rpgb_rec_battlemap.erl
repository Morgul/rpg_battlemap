...
make_json(Req, Host, Port, Map, Layers, Combatants) ->
	...
	MakeLayerJson = fun(InJson, _InMap) ->
		LayersJsons = [rpgb_rec_layer:make_json(Req, Host, Port, Layer) || Layer <- Layers],
		[{layers, LayersJsons} | InJson]
	end,
	MakeCombatantJson = fun(InJson, _InMap) ->
		CombatantsJsons = [rpgb_rec_combatant:make_json(Req, Host, Port, Combatant) || Combatant <- Combatants],
